...
- **Long-running waiters** - `pipelines_wait` / `jobs_wait` are non-blocking: they register a background poll task and return a `wait_id` + first snapshot immediately. The task streams `wait_transition` (and, for pipelines, `wait_stage_transition`) log events plus a final `wait_terminal` notification — carrying a per-stage summary — to the client over the MCP reverse (server→client) stream. `pipelines_wait_poll(wait_id, max_block=...)` / `*_wait_cancel` / `waits_list` and the `gitlab://waits/{id}` resource are the fallback when no stream is open. The returned snapshot is always the source of truth; reverse-stream delivery is best-effort and reported via `notified` / `notify_error`. Waits tolerate transient API errors (`max_poll_failures`, default 3 consecutive) and background waits self-terminate after `max_lifetime` (default 2 h)
